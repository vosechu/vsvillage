...
public class AiTaskVillagerSleep : AiTaskBase
{
    ...
    public override void StartExecute()
    {
        ...
        else
        {
            ...
            // Tier-3 fallback: teleport-to-safe before giving up; otherwise villagers bounce between failed evals.
            TeleportToSafeLocation();
            if (reachedBed)
            {
                // nothing to do, ContinueExecute will handle the Lie animation
            }
    ...
    private Vec3d GetBedStandingPos(Vec3d bedCenter)
    {
        ...
        foreach (BlockFacing facing in BlockFacing.HORIZONTALS)
        {
            ...
            // Scan downward within step-height range (villager step height 1.2 = 1 block).
            // Handles beds elevated on platforms where nb at bed Y has air below it.
        ...
        entity.World.Logger.Warning("[VsVillage] Sleep: no clear position around bed " + bedBlockPos + " - using bed center");
        return bedCenter;
    }

    // Scan down from origin up to scanDepth blocks; first clear+headclear+grounded wins.
    private Vec3d FindGroundedStandingPos(IBlockAccessor ba, BlockPos origin, int scanDepth)
    {
    ...
    private bool IsPositionSafe(Vec3d pos)
    {
        ...
        IBlockAccessor ba = entity.World.BlockAccessor;
        return FindGroundedStandingPos(ba, pos.AsBlockPos, scanDepth: 1) != null;
    }
